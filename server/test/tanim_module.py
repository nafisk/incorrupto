...
async def analyze_text(text, headers):
    async with aiohttp.ClientSession() as session:
        fact_opinion_future = query_huggingface_async(
            session,
            "https://api-inference.huggingface.co/models/lighteternal/fact-or-opinion-xlmr-el",
            {"inputs": text},
            headers,
        )
        toxicity_future = query_huggingface_async(
            session,
            "https://api-inference.huggingface.co/models/unitary/unbiased-toxic-roberta",
            {"inputs": text},
            headers,
        )
        implicit_hate_future = query_huggingface_async(
            session,
            "https://api-inference.huggingface.co/models/tomh/toxigen_roberta",
            {"inputs": text},
            headers,
        )

        results = await asyncio.gather(
            fact_opinion_future, toxicity_future, implicit_hate_future
        )
        return results
